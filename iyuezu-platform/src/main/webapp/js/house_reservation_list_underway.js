...
function goToReservationListAll() {
	var url = "/house_reservation_list.html";
	redirect(url);
}
...
function getOperatorHtml(status, reservationUuid, houseUuid) {
	...
	if (status == 1) {
		...
		operatorHtml += ("<a href='javascript:void(0);' onclick='rentHouse(\"" + houseUuid + "\");' class='link'>确定租房</a>");
	} else if (status == 5) {
		operatorHtml += ("<a href='javascript:void(0);'></a>");
		operatorHtml += ("<a href='javascript:void(0);' onclick='rentHouse(\"" + houseUuid + "\");' class='link'>确定租房</a>");
	}
...
function getOperatorHtml(status, reservationUuid, houseUuid) {
	if (admin == 1) {
		...
		if (status == 1) {
			...
			operatorHtml += ("<a href='javascript:void(0);' onclick='cancelReservation(\"" + reservationUuid + "\");' class='link'>取消预约</a>");
			operatorHtml += ("<a href='javascript:void(0);' onclick='showConfirmDialog(\"" + reservationUuid + "\");' class='link'>确认信息</a>");
		} else if (status == 4) {
			operatorHtml += ("<a href='javascript:void(0);'></a>");
			operatorHtml += ("<a href='javascript:void(0);' onclick='goToCommentEdit(\"" + houseUuid + "\");' class='link'>马上评论</a>");
		}
